...
export const initSocketServer = async (httpServer) => {
  ...
  io.on("connection", async (socket) => {
    try {
      ...
      allMessages.push(...dbMessages.map(msg => ({ ...msg, source: "db" }))
      );
      ...
      for (const msgStr of redisMessages) {
        try {
          const msg = JSON.parse(msgStr); allMessages.push({ ...msg, source: "redis" });
        } catch (e) {
      ...
      setTimeout(async () => {
        for (const msg of allMessages) {
          ...
          socket.emit("chat_receive", {
            ...msg,
            timestamp: Number(msg.timestamp),
            encrypted_payload: msg.encrypted_payload ? msg.encrypted_payload.toString() : null
          });
        }
      }, 500);
